...
import { aggregateByWeekday, mergeDailyAndEquity, aggregateMonthly } from './Overview.helpers';
...
function PnlByWeekday({ dailyData = [] }) {
  const weekdayData = useMemo(() => aggregateByWeekday(dailyData), [dailyData]);
...
function ComboPnlChart({ dailyData = [], equityData = [] }) {
  const merged = useMemo(() => mergeDailyAndEquity(dailyData, equityData), [dailyData, equityData]);
